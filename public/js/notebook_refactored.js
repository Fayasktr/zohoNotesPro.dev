...
class NotebookApp {
    ...
    setupEventListeners() {
        // Explicit listeners for main action buttons
        document.getElementById('add-code-cell').addEventListener('click', () => this.addCell('code'));
        document.getElementById('add-markdown-cell').addEventListener('click', () => this.addCell('markdown'));
        ...
        // Sidebar Header Click (Toggle)
        document.getElementById('toggle-notebooks').addEventListener('click', (e) => {
            // Only toggle if not clicking the add button
            if (e.target.closest('#add-notebook-sidebar')) return;
        ...
        // Add Notebook in Sidebar (Explicit)
        document.getElementById('add-notebook-sidebar').addEventListener('click', (e) => {
            e.stopPropagation();
        ...
        // EVENT DELEGATION for Notebook List (Sidebar)
        ...
        // EVENT DELEGATION for Cells (Main List)
    ...
    renderNotebookList(notebooks) {
        ...
        Object.keys(groups).sort().forEach(folder => {
            if (folder !== 'root') {
                ...
                folderHeader.innerHTML = `<i data-lucide="folder" style="width: 14px;"></i> ${folder}`;
            ...
            groups[folder].forEach(nb => {
                ...
                item.innerHTML = `
                    <i data-lucide="${nb.id === this.notebook.id ? 'edit-3' : 'file-text'}" style="width: 14px;"></i> 
    ...
    renderCell(cell) {
        ...
        require(['vs/editor/editor.main'], () => {
            ...
            const updateHeight = () => {
                ...
                const newHeight = Math.max(isMark ? 60 : 80, lineCount * 19 + 25);
                document.getElementById(`editor-${cell.id}`).style.height = `${newHeight}px`;
                editor.layout();
            };
